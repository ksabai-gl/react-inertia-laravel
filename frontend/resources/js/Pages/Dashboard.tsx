...
const statusOptions = ['active', 'paused', 'failed'] as Status[];
...
export default function Dashboard({
    ...
    const [moduleFilter, setModuleFilter] = useState<ActivityFilter>('all');
    const [statusFilter, setStatusFilter] = useState<ActivityFilter>('all');
    const [regionFilter, setRegionFilter] = useState<ActivityFilter>('all');

    const moduleOptions = useMemo(
        () => [...new Set(activity.map((row) => row.module))].sort(),
        [activity],
    );
    const regionOptions = useMemo(
        () => [...new Set(activity.map((row) => row.region))].sort(),
        [activity],
    );
    const filteredActivity = useMemo(
        ...
        [activity, moduleFilter, statusFilter, regionFilter],
    );
                            ...
                            <div className="grid gap-3 sm:grid-cols-3">
                                <label className="text-muted-foreground flex flex-col gap-1 text-xs font-medium">
                                    ...
                                    <select
                                        ...
                                        className="border-input bg-background text-foreground h-9 rounded-md border px-3 text-sm font-normal"
                                    >
                                ...
                                <label className="text-muted-foreground flex flex-col gap-1 text-xs font-medium">
                                    ...
                                    <select
                                        ...
                                        className="border-input bg-background text-foreground h-9 rounded-md border px-3 text-sm font-normal capitalize"
                                    >
